...
// TRIGGER EVENT (TO AVOID REWRITE CODE)
  ...
  function formatSearchResultItem(item, terms) {
    ...
    li.classList.add("w3-hover-light-blue");                            // HOVER COLOR CLASS
    li.innerHTML = `<a href="${item.ref}"><h4>${item.doc.title}</h4></a>`;
  ...
  function initSearch() {
    ...
    // TODO REPLACE BY BETTER VALUE
    elasticlunr.Pipeline.registerFunction(x=>x, "trimmer-fr") 
    elasticlunr.Pipeline.registerFunction(x=>x, "stopWordFilter-fr")
    elasticlunr.Pipeline.registerFunction(x=>x, "stemmer-fr")
...
// INIT RESEARCH IF QUERY STRING IS DEFINED
const urlParams = new URLSearchParams(window.location.search);
const search_string = urlParams.get('s');
  if (search_string) {
